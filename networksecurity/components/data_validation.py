...
class DataValidation:   
    ...
    def validate_num_of_columns(self,dataframe:pd.DataFrame)->bool:
        try:
            ...
            raise NetworkSecurityException(e,sys)
    def validate_numerical_column_exist(self,dataframe:pd.DataFrame)->bool:
        try:
            logging.info(f"the dataframe has following column{dataframe.columns} with datatype {dataframe.columns.dtype}")
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            if not status:
                error_message=f"train file is not having all columns"
                logging.info(error_message)
            status=self.validate_num_of_columns(test_df)
            if not status:
                error_message=f"test file is not having all columns"
                logging.info(error_message)
